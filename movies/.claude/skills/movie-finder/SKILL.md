...
El script está en `scripts/movies.sh` dentro de la skill. Ejecútalo siempre desde su ruta relativa:

```bash
bash scripts/movies.sh <comando> [opciones]
```
...
```bash
bash scripts/movies.sh search --query "Matrix"
```

Con idioma en español:
```bash
bash scripts/movies.sh search --query "Matrix" --language es-MX
```
...
```bash
bash scripts/movies.sh details --id 603
```

Con videos y créditos incluidos:
```bash
bash scripts/movies.sh details --id 157336 --append videos,credits
```
...
```bash
bash scripts/movies.sh details --id {id} --append credits
```
