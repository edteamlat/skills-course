...
```bash
./scripts/movies.sh search --query <texto> [opciones]
```
...
```bash
# Búsqueda simple
./scripts/movies.sh search --query "Jack Reacher"

# Con idioma español y página 2
./scripts/movies.sh search --query "Matrix" --language es-MX --page 2

# Filtrar por año de estreno
./scripts/movies.sh search --query "Spider-Man" --primary-release-year 2021

# Filtrar por región
./scripts/movies.sh search --query "Avengers" --region US
```
...
```bash
./scripts/movies.sh discover [opciones]
```
...
./scripts/movies.sh discover --with-genres 28 --vote-average-gte 8 --sort-by vote_average.desc

# Películas en español estrenadas en 2024
./scripts/movies.sh discover --with-original-language es --primary-release-year 2024

# Películas cortas (menos de 90 min) disponibles en Netflix (ID: 8)
./scripts/movies.sh discover --runtime-lte 90 --with-watch-providers 8 --watch-region US

# Películas de terror entre 2020 y 2023
./scripts/movies.sh discover --with-genres 27 --release-date-gte 2020-01-01 --release-date-lte 2023-12-31

# Películas con más de 1000 votos y puntuación entre 7 y 9
./scripts/movies.sh discover --vote-count-gte 1000 --vote-average-gte 7 --vote-average-lte 9
```
...
```bash
./scripts/movies.sh details --id <movie_id> [opciones]
```
...
./scripts/movies.sh details --id 11

# Detalles en español
./scripts/movies.sh details --id 11 --language es-MX

# Detalles con videos y créditos incluidos
./scripts/movies.sh details --id 11 --append videos,credits

# Detalles completos
./scipts/movies.sh details --id 343611 --append videos,images,credits,keywords,recommendations
```
...
```bash
./scripts/movies.sh find --id <external_id> --source <fuente> [opciones]
```
...
./scripts/movies.sh find --id tt0816692 --source imdb_id

# Buscar The Matrix en español
./scripts/movies.sh find --id tt0133093 --source imdb_id --language es-MX

# Buscar por ID de Wikidata
./scripts/movies.sh find --id Q47703 --source wikidata_id
```
...
```bash
./scripts/movies.sh --help
./scripts/movies.sh search --help
./scripts/movies.sh discover --help
./scripts/movies.sh details --help
./scripts/movies.sh find --help
```
